...
takeaways = [
  ...
  "PRAGMA delivers +130% PR-AUC on credit scoring and +163% AUUC on uplift via LoRA fine-tuning of a shared backbone, per the figures reported in the paper.",
  "The paper itself reports a 47.1% drop on the F-0.5 anti-money-laundering downstream task, attributed by the authors to PRAGMA processing user histories in isolation rather than across users; this is a property of the research backbone, not a statement about Revolut's production AML stack.",
  "Nubank's nuFormer reaches a similar conclusion via a different architecture, suggesting the field is converging.",
]
faq = [
  ...
  {question = "Why does the paper report a drop on the AML downstream task?", answer = "AML is a relational problem: detection patterns run across users and across accounts. The PRAGMA backbone, as described in the paper, processes each user history in isolation, and the authors themselves report a 47.1% drop on F-0.5 on the AML downstream task versus a task-specific baseline. The lesson the paper draws generalises: per-user encoders are not graph-aware, and graph-aware components needed for AML detection are a complement to such backbones, not something they can replace. This is a property of the research backbone in the paper, not a statement about Revolut's production AML stack."},
