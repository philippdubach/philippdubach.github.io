...
aliases = ["/2025/05/30/gambling-vs.-investing/"]
takeaways = [
...
faq = [
  {question = "Is Kalshi sports betting or investing?", answer = "Kalshi operates as a CFTC-regulated exchange where traders buy and sell contracts on event outcomes, including sports. While it is technically a financial exchange rather than a sportsbook taking bets against customers, 79% of its recent trading volume has been sports-related, making the distinction largely semantic."},
  {question = "How does Kalshi offer sports betting in states where it is illegal?", answer = "Kalshi holds a federal financial license from the CFTC as a Designated Contract Market. The company argues that federal regulation preempts state gambling laws, allowing it to offer event contracts nationwide. Several states have issued cease-and-desist letters challenging this position."},
  {question = "What is the difference between gambling and investing?", answer = "Traditional definitions distinguish them by expected returns, time horizon, and asset ownership. However, many financial instruments like oil futures and stock options share characteristics with gambling. The line is increasingly arbitrary when regulated exchanges let you wager on NFL touchdowns using the same mechanics as commodity futures."},
  {question = "Why did the CFTC drop its legal challenge to Kalshi?", answer = "Under the Trump administration's deregulatory approach, the CFTC dropped its appeal against Kalshi's election contracts. The agency has since withdrawn its proposed ban on sports and political event contracts, and a Kalshi board member is awaiting confirmation to lead the CFTC."},
]
+++
