...
faq = [
  {question = "Why is OpenAI losing billions despite growing revenue?", answer = "Press reporting (via The Register's reading of Microsoft's quarterly filings) has attributed a single-quarter net loss in the range of $11.5 billion to OpenAI, with the company reportedly spending roughly $5 to deliver each dollar of revenue. These figures are press-sourced rather than from OpenAI's audited disclosures. The reported losses are framed as the cost of overlapping profitable model cycles with investment in each successive generation, where each new model is reported to cost approximately 10x more than the last to train."},
...
+++
> Microsoft reported earnings for the quarter ended Sept. [...] buried in its financial filings were a couple of passages suggesting that OpenAI suffered a net loss of $11.5 billion or more during the quarter (per press reporting; figure is press-sourced rather than from audited disclosures).
